function Consolidate_sheet_workLM() {
  common_consolidateData("worklm");
}

function Consolidate_sheet_Autonomous() {
  common_consolidateData("autonomous");
}

function common_consolidateData(option) {
  try {
      Logger.log(option);
   ...
   // var testCasesFormatSheet = ss.getSheetByName("Test Cases Format");
    ...
    var testCasesFormatSheet;
    if (option.toLowerCase() === "worklm") {
      testCasesFormatSheet = ss.getSheetByName("WorkLM_Test Cases Format");
    } else if (option.toLowerCase() === "autonomous") {
      testCasesFormatSheet = ss.getSheetByName("Autonomous_Test Cases Format");
    } else {
...
if (consolidatedSheet) {
  ...
  if (response === "yes") { 
    // Clear existing data if the user explicitly chose "Yes"
    consolidatedSheet.clearContents();
    Logger.log("Existing data cleared in 'Consolidated Data' sheet.");
  } else {
    // Treat "No" or dismissals (clicking outside) as stopping the process
    Logger.log("Consolidation process stopped by the user.");
    ss.toast("Process stopped. No changes made to 'Consolidated Data'.", " ⚠️ Process Terminated", 10);
    return;
  }
} else {
  // Create the sheet if it does not exist
  consolidatedSheet = ss.insertSheet("Consolidated Data");
  Logger.log("'Consolidated Data' sheet created successfully.");
}
...
// SpreadsheetApp.getActiveSpreadsheet().toast("Policy sheet '" + policyName + "' not found.", "Warning", 5);
  ...
  if (policyDataValues.length <= 1) { // Check if the sheet only has headers or no data
    Logger.log("Policy sheet '" + policyName + "' has no data.");
    consolidatedSheet.appendRow([policyName, "<sheet with No data available>"]);
    continue;
  }
var policyHeaders = policyDataValues[0];
    ...
    var ruleHeaderNotFound = SpreadsheetApp.newConditionalFormatRule()
  ...
  var ruleSheetNotFound = SpreadsheetApp.newConditionalFormatRule()
      .whenTextContains("<sheet")
      .setBackground("#ffcccc") // Red color for sheet name not found
      .setRanges([range])
      .build();


    var rules = consolidatedSheet.getConditionalFormatRules();
    rules.push(ruleHeaderNotFound,ruleSheetNotFound);
