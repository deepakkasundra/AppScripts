...
Domain Domain = Domain + '/<REDACTED_PATH>/' + Domain  + Domain(Domain,'Domain Domain Domain');
  ...
  try {
    ...
    const msg = 'Error fetching link for form ID ' + formId + ':\n' + error;
    Logger.log(msg);
  
     // Log the failure
    logLibraryUsage('Fetch tkt form getLinkForFormId ' + formId, 'Fail', error.toString());
  
     // Show toast popup in the sheet
      SpreadsheetApp.getActiveSpreadsheet().toast(msg, "Fetch Error", 5); // 5 seconds
